                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    @if(auth()->user()->is_admin)
                        ...
                        <x-dropdown align="left" width="w-96">
                            ...
                            <x-slot name="content">
                                @if(($adminPoList ?? collect())->isEmpty())
                                    <div class="px-4 py-3 text-sm text-gray-500">{{ __('Belum ada data PO.') }}</div>
                                @else
                                    <div class="max-h-96 overflow-y-auto divide-y divide-gray-100">
                                        @foreach($adminPoList as $poItem)
                                            <a href="{{ $poItem['url'] }}" class="block px-4 py-3 hover:bg-gray-50 transition">
                                                <div class="flex items-start justify-between gap-3">
                                                    <div class="min-w-0">
                                                        <p class="text-sm font-medium text-gray-800 truncate">{{ $poItem['title'] }}</p>
                                                        <p class="text-xs text-gray-500 truncate">{{ $poItem['subtitle'] }}</p>
                                                    </div>
                                                    <span class="inline-flex items-center shrink-0 px-2 py-0.5 rounded-full text-[11px] font-semibold bg-indigo-50 text-indigo-700" title="{{ $poItem['po_number'] }}">
                                                        {{ $poItem['po_number'] }}
                                                    </span>
                                                </div>
                                            </a>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            @if(auth()->user()->is_admin)
                ...
                @if(($adminPoList ?? collect())->isNotEmpty())
                    <div class="px-4 py-2 text-xs font-semibold uppercase tracking-wide text-gray-400">{{ __('Daftar PO') }}</div>
                    @foreach($adminPoList->take(5) as $poItem)
                        <a href="{{ $poItem['url'] }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                            <div class="flex items-start justify-between gap-2">
                                <span class="truncate">{{ $poItem['title'] }}</span>
                                <span class="text-[11px] font-semibold text-indigo-700 shrink-0">{{ $poItem['po_number'] }}</span>
                            </div>
                        </a>
                    @endforeach
                @endif
